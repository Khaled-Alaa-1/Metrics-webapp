...
const Details = () => {
  ...
  const countries = useSelector((state) => state.data.countries.filter((country) => country.continent === params.country_name));
  ...
  useEffect(() => {
    if (countries.length > 0) { // Check for countries array length
      setTempRegions(countries[0].countries);
    }
  }, [countries]);
    ...
    <>
      ...
      <div className="flex justify-around items-center py-6 bg-sky-500">
        ...
        <div className="flex flex-col items-end text-3xl font-semibold text-white">
          ...
          <span>{`${countries[0].cases.toLocaleString('en-US')} cases`}</span>
        </div>
      </div>
      )}
